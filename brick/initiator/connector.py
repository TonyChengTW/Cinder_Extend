...
class FibreChannelConnector(InitiatorConnector):
    ...
    def connect_volume(self, connection_properties):
        ...
        # Note(luqitao): There are 2 possible scenario:
        # 1.he size of two luns sometimes are not the same
        # so i have to force refresh the size of them
        # TODO: need to consider fiber chanel controller and the wire was broken
        # 2.sometimes rescan hosts cannot find the 2 devices, due to
        # os system was busy to do somethings.
        disk_paths = self._linuxscsi.find_disk_path_by_wwn(device_wwn)
        for path in disk_paths:
            self._linuxscsi.rescan_device(path)

        putils.execute("multipath", "-v2", device_wwn)
        ...
        if path is None:
            ...
            if mdev_info is not None:
                ...
                device_info['devices'] = mdev_info['devices']
                device_info['multipath_id'] = mdev_info['id']
            else:
                # Note(luqitao): instance create will failed, we can not use
                # only 1 device.
                LOG.error("Unable to find multipath device name %(device_wwn)s"
                          "for volume. ",
                          {'device_wwn': device_wwn})
                raise exception.VolumeDeviceNotFound(device=self.device_name)
        else:
            device_info['path'] = path
            device_info['multipath_id'] = device_wwn
            dev_info = self._linuxscsi.get_device_info(self.device_name)
            device_info['devices'] = [dev_info]
        return device_info
    ...
    def disconnect_volume(self, connection_properties, device_info):
        ...
        if self.use_multipath:
            ...
            # we'll try anyway.
            wwn = device_info.get('multipath_id')
            if wwn:
