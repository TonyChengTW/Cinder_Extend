...
class BaseVD(object):
    ...
    def _detach_volume(self, context, attach_info, volume, properties,
        ...
        # NOTE(luqitao) There is a bug that the thread will be switched
        # and another thread can rescan fc_host between disconnect_volume
        # and terminate_connection, so we change the order
        # delete local device after terminate_connection from backend
        ...
        if remote:
            ...
            try:
                ...
                raise exception.RemoveExportException(volume=volume['id'],
                                                      reason=ex)
        # Use Brick's code to do attach/detach
        LOG.debug('[luqitao] start disconnect_volume, attach_info: %s'
                  % attach_info)
        connector = attach_info['connector']
        connector.disconnect_volume(attach_info['conn']['data'],
                                    attach_info['device'])
    ...
    def _attach_volume(self, context, volume, properties, remote=False):
        ...
        except exception.DeviceUnavailable as e:
            LOG.error(e.msg)
            with excutils.save_and_reraise_exception():
                if remote:
                    # Call remote manager's terminate_connection which includes
    ...
    def _connect_device(self, conn):
        ...
        if not connector.check_valid_device(host_device, root_access):
            connector.disconnect_volume(conn['data'], device)
